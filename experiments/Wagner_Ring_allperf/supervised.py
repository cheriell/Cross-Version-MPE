...
resume_from_checkpoint_path = '/home/lel79bc/workspace/cross-version-mpe/experiments/Wagner_Ring_allperf/models/supervised.pt'
...
train_dataset_params = {'context': 75,
                        # 'stride': 50,
                        'stride': 50 * 16,  # change to a more sparse stride for training
                        'compression': 10,
...
val_dataset_params = {'context': 75,
                    #   'stride': 50,
                      'stride': 500,  # change to a more sparse stride for validation
                      'compression': 10
                      }
...
# Supervised training: Use ground truth annotations
path_annot = path_annot_test
...
path_dataset_buffer_y = os.path.join('/home/lel79bc/workspace/cross-version-mpe/experiments', dataset, 'dataset_buffer', 'y_groundtruth')
    ...
    for fn in os.listdir(path_data):
        if any(train_version in fn for train_version in train_versions) and \
            ...
            if save_dataset_buffer:
                if not os.path.exists(fn_buffer_X+'_inputs_seg_0.npy'):
            ...
            if save_dataset_buffer:
                ...
                seg_idx_list = range(np.ceil(targets.shape[0] / 5000).astype(int))

                def save_segment(seg_idx):
                # for seg_idx in seg_idx_list:
                    print('Saving segment %d of %d' % (seg_idx+1, np.ceil(targets.shape[0] / 5000).astype(int)))
            ...
            # Add to dataset
            curr_dataset = dataset_context_fix_length_and_buffer_segmentwise(inputs=None, targets=None, params=train_dataset_params, fn=[fn_buffer_X, fn_buffer_y, fn_buffer_other], inputs_len=inputs_len)
        ...
        if do_val:
            if any(val_version in fn for val_version in val_versions) and \
                ...
                # Add to dataset
                curr_dataset = dataset_context_fix_length_and_buffer_segmentwise(inputs=None, targets=None, params=val_dataset_params, fn=[fn_buffer_X, fn_buffer_y, fn_buffer_other], eval=True, inputs_len=inputs_len)
