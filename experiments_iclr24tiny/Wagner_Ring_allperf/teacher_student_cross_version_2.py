...
# Multiprocessing does not support running in main()

################################################################################
#### Set experimental configuration ############################################
################################################################################
version_str = "v2.m0.p1-repeat2"
average_labels_folder = 'two_version_average_labels.eval_thresh=0.50'
...
if do_train:
    ...
    max_epochs = 50 # 100 change to 50
            ...
            for fn_averaged_labels_path in os.listdir(two_version_average_labels_path):
                if fn_averaged_labels_path.startswith(fn):
                    ...
                    if valid_alignment:
                        ...
                        fn_buffer_y = os.path.join(path_dataset_buffer_y, fn_averaged_labels_path)
                        fn_buffer_other = os.path.join(path_dataset_buffer_other, fn_averaged_labels_path)
                        ...
                        # Add to dataset
                        curr_dataset = dataset_context_fix_length_and_buffer_segmentwise(inputs=None, targets=None, params=train_dataset_params, fn=[fn_buffer_X, fn_buffer_y, fn_buffer_other], agreed_index=pitched_frames_idx, inputs_len=inputs_len)
                ...
                for fn_averaged_labels_path in os.listdir(two_version_average_labels_path):
                    if fn_averaged_labels_path.startswith(fn):
                        ...
                        if valid_alignment:
                            ...
                            # Add to dataset
                            curr_dataset = dataset_context_fix_length_and_buffer_segmentwise(inputs=None, targets=None, params=val_dataset_params, fn=[fn_buffer_X, fn_buffer_y, fn_buffer_other], agreed_index=pitched_frames_idx, eval=True, inputs_len=inputs_len)
