...
dir_models = os.path.join('/mnt/d/workspace/cross-version-mpe/experiments/models')
...
dir_output = os.path.join('/mnt/d/workspace/cross-version-mpe/experiments/results_filewise')
...
dir_predictions = os.path.join('/mnt/d/workspace/cross-version-mpe/experiments/predictions', expname)
...
path_log = os.path.join('/mnt/d/workspace/cross-version-mpe/experiments/logs', fn_log)
...
metadata = pd.read_csv(os.path.join('/mnt/d/Datasets/original_datasets/MAESTRO/maestro-v3.0.0.csv'))
all_fn = set(os.listdir(path_data))
for i, row in metadata.iterrows():
    if row['audio_filename'][5:-4]+'.npy' in all_fn:
        if row['split'] == 'train':
            all_train_fn.append(row['audio_filename'][5:-4]+'.npy')
        elif row['split'] == 'validation':
            all_val_fn.append(row['audio_filename'][5:-4]+'.npy')
        elif row['split'] == 'test':
            all_test_fn.append(row['audio_filename'][5:-4]+'.npy')
print('Train files: ' + str(len(all_train_fn)))
print('Val files: ' + str(len(all_val_fn)))
print('Test files: ' + str(len(all_test_fn)))

path_dataset_buffer = os.path.join('/mnt/d/workspace/cross-version-mpe/experiments/dataset_buffer', expname)
if not os.path.exists(path_dataset_buffer):
    os.makedirs(path_dataset_buffer)

if do_train:
    for i, fn in enumerate(all_train_fn):
        print('{}/{}'.format(i+1, len(all_train_fn)))
        ...
        curr_dataset = dataset_context(inputs, targets, train_dataset_params, fn=os.path.join(path_dataset_buffer, fn))
        all_train_sets.append(curr_dataset)
        logging.info(' - file ' + str(fn) + ' added to training set.')
if do_val:
    for i, fn in enumerate(all_val_fn):
        print('{}/{}'.format(i+1, len(all_val_fn)))
        inputs = torch.from_numpy(np.transpose(np.load(os.path.join(path_data, fn)), (2, 1, 0)))
        targets = torch.from_numpy(np.load(os.path.join(path_annot, fn)).T)
        if num_output_bins!=12:
            targets = targets[:, min_pitch:(min_pitch+num_output_bins)]
        curr_dataset = dataset_context(inputs, targets, val_dataset_params, fn=os.path.join(path_dataset_buffer, fn))
...
#### START TRAINING ############################################################
    ...
    for epoch in range(max_epochs):
        ...
        for i, (local_batch, local_labels) in enumerate(train_loader):
            ...
            print('Training step: {}/{}'.format(i+1, len(train_loader)))
            # Transfer to GPU
        ...
        if do_val:
            ...
            for j, (local_batch, local_labels) in enumerate(val_loader):
                ...
                print('Validation step: {}/{}'.format(j+1, len(val_loader)))
                # Transfer to GPU
                local_batch, local_labels = local_batch.to(device), local_labels.to(device)
...
if do_test:
    ...
    if True:
        ...
        for test_id, fn in enumerate(all_test_fn):
            print('Testing file {}/{}'.format(test_id+1, len(all_test_fn)))
            ...
            if True:
                ...
                for i, (test_batch, test_labels) in enumerate(test_generator):
                    print('   Testing step: {}/{}'.format(i+1, len(test_generator)))
        ...
        logging.info('#   Results for test set  ########################################## \n')
