...
def main(args):
    ...
    average_labels_folder = args.average_labels_folder
    dataset = args.dataset
    mask_disagree = args.mask_disagree
    pitched_only = args.pitched_only
    ...
    # Use the average teacher annotations as training annotations
    two_version_average_labels_path = os.path.join('/home/lel79bc/workspace/cross-version-mpe/feature_preparation', dataset, average_labels_folder)
    ...
    path_dataset_buffer = os.path.join('/home/lel79bc/workspace/cross-version-mpe/experiments', dataset, 'dataset_buffer', expname)
    ...
    if do_train:
        for fn in os.listdir(path_data):
            if any(train_version in fn for train_version in train_versions) and \
                ...
                # Get average labels
                for fn_averaged_labels_path in os.listdir(two_version_average_labels_path):
                    if fn_averaged_labels_path.startswith(fn):
                        ...
                        for version in val_versions + test_versions:
                            if version in fn_averaged_labels_path:
                                valid_alignment = False
                        if valid_alignment:

                            # Get target annotation
                            targets = torch.from_numpy(np.load(os.path.join(two_version_average_labels_path, fn_averaged_labels_path))).float()

                            if pitched_only:
                                # Get pitched frames (i.e. frames with at least one pitch active, i.e. at least a one in the target column)
                                targets_ignore_half = (targets > 0.5).int()
                                pitched_frames = torch.sum(targets_ignore_half, dim=1) > 0
                                # convert into index list
                                pitched_frames_idx = torch.nonzero(pitched_frames).squeeze()
                                
                            # Add to the dataset
                            curr_dataset = dataset_context(inputs, targets, train_dataset_params, fn=os.path.join(path_dataset_buffer, fn_averaged_labels_path), agreed_index=pitched_frames_idx)
                    ...
                    for fn_averaged_labels_path in os.listdir(two_version_average_labels_path):
                        if fn_averaged_labels_path.startswith(fn):
                            ...
                            if valid_alignment:
                                ...
                                # Add to the dataset
                                curr_dataset = dataset_context(inputs, targets, val_dataset_params, fn=os.path.join(path_dataset_buffer, fn_averaged_labels_path), agreed_index=pitched_frames_idx)
    ...
    #### START TRAINING ############################################################
        ...
        for epoch in range(max_epochs):
            ...
            for i, (local_batch, local_labels) in enumerate(train_loader):
                ...
                if mask_disagree:
                    mask = (local_labels != 0.5).float()
                    y_pred = y_pred * mask
                    local_labels = local_labels * mask
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run experiment TS-CV2')
    parser.add_argument('--do_train', type=bool, default=True, help='Whether to train the model')
    parser.add_argument('--do_val', type=bool, default=True, help='Whether to validate the model')
    parser.add_argument('--do_test', type=bool, default=True, help='Whether to test the model')

    parser.add_argument('--version_str', type=str, help='Version string of the experiment, e.g. v0.0-repeat0')
    parser.add_argument('--average_labels_folder', type=str, help='Folder containing the agreed frames from the teacher annotations')
    
    parser.add_argument('--dataset', type=str, default='Schubert_Winterreise', help='Dataset to use for the experiment')

    parser.add_argument('--mask_disagree', type=int, default=0, help='Whether to mask the disagreed frames (0.5 in average) in the teacher annotations')
    parser.add_argument('--pitched_only', type=int, default=0, help='Whether to use only pitched frames for training')
