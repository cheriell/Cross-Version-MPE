...
def main(args):
    ...
    agreed_frames_folder = args.agreed_frames_folder
    ...
    if do_train:
        for fn in os.listdir(path_data):
            if any(train_version in fn for train_version in train_versions) and \
                ...
                targets = torch.from_numpy(np.load(os.path.join(path_annot, fn)) > eval_thresh).float()
                ...
                for fn_agreed_index in os.listdir(path_agreed_index):
                    if fn_agreed_index.startswith(fn):
                        ...
                        if valid_alignment:
                            ...
                            # Add to dataset
                            curr_dataset = dataset_context(inputs, targets, train_dataset_params, fn=os.path.join(path_dataset_buffer, fn_agreed_index), agreed_index=agreed_index)
                    ...
                    for fn_agreed_index in os.listdir(path_agreed_index):
                        if fn_agreed_index.startswith(fn):
                            ...
                            if valid_alignment:
                                ...
                                # Add to dataset
                                curr_dataset = dataset_context(inputs, targets, val_dataset_params, fn=os.path.join(path_dataset_buffer, fn_agreed_index), agreed_index=agreed_index, eval=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--version_str', type=str, default='v0.0', help='Version string of the experiment, e.g. v0.0-repeat0')
    parser.add_argument('--agreed_frames_folder', type=str, help='Folder containing the agreed frames from the teacher annotations')
